...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    error_reporting(0);
     // Verifica se o usuário não está logado e redireciona para a página de login:
        ...
        // Obtém o valor do cookie 'cpts' que parece ser a pontuação:
        $points = $_COOKIE['cpts'];
       // Faz a verificação se a conexão com o banco de dados foi estabelecida:
        if($conexao == false)
            echo "<script>alert('Erro ao conectar ao banco de dados!')</script>";
        else {
            // Executa uma consulta para obter todos os usuários ordenados por código:
            ...
            while($row = mysqli_fetch_assoc($res)) {
                if ($row["codigo"] == $logado){
                    // Atualiza a pontuação do usuário logado:
...
<head>
    ...
    <!--css-->
    <link rel="stylesheet" href="jogarfacil.css"> 
    <link rel="stylesheet" href="font.css">
    <title>Jogar fácil</title>
...
<body onload="gerarStrings()" oncopy="return false;" oncut="return false;" onpaste="return false;" oncontextmenu="return false;">
    <form id="jogar" name="jogar" method="post" action="jogarfacil.php">
        <div id="principal">
     ...
     <!--JS-->
     <script src="jogarfacil.js"></script>
</body>
</html>
